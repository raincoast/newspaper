...
import type { ApartmentGroupOverlay, DeliveryStatus, HouseMarkerDTO, RegionLite } from "./types"
import {
...
export default function MapView({
  ...
  useEffect(() => {
    ...
    // Ensure nearby overlay click is bound (only binds once per map instance)
    bindHouseMarkerClick(map, (markerId) => setSelectedMarkerId(markerId))
  ...
  return (
    <div className="relative h-[calc(100vh-9.5rem)] w-full overflow-hidden rounded-2xl border border-black/10">
      ...
      <div className="pointer-events-none absolute left-3 right-3 top-3 z-20 flex items-start justify-between gap-2">
        ...
        <div className="pointer-events-auto w-[210px]">
          <StatsCard markers={markers} />
        </div>
      </div>

      {nextMarker ? (
        <div className="pointer-events-none absolute left-3 top-[78px] z-30 rounded-xl bg-green-600 px-3 py-2 text-white shadow-sm">
          <div className="text-[11px] opacity-90">下一个投递门牌</div>
          <div className="text-lg font-bold">
            {nextMarker.marker.current_housenumber}
          </div>
        </div>
      ) : null}

      {geoError ? (
        <div className="pointer-events-none absolute left-3 right-3 top-[70px] z-30 rounded-xl bg-white/80 p-2 text-xs text-red-600 shadow-sm backdrop-blur">
          {geoError}
        </div>
      ) : null}

      {userLocation ? (
        <div className="pointer-events-auto absolute right-3 bottom-24 z-30">
          <button
            type="button"
            onClick={backToMe}
            className={[
              "rounded-xl bg-white/90 px-3 py-2 text-sm text-black shadow-sm backdrop-blur",
      ...
      <div className="pointer-events-auto absolute bottom-24 left-3 z-30">
        <button
          ...
          className="rounded-xl border border-black/10 bg-white/90 px-3 py-2 text-sm text-black shadow-sm backdrop-blur"
        >
          i 今日信息
        </button>
      </div>
      ...
      {showInfo ? (
        <div className="pointer-events-none absolute bottom-36 left-3 z-30 rounded-xl bg-white/90 p-3 text-xs text-black shadow-sm backdrop-blur">
          <div>今天已投递：{markers.filter((m) => m.delivery_status === "delivered").length}</div>
          <div>
            今天应投递：
            {markers.filter((m) => m.delivery_status !== "blocked").length}
          </div>
        </div>
      ) : null}
